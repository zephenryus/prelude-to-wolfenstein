...
import com.example.escape.level.block.*;
import com.example.escape.menu.GotLootMenu;
...
public abstract class Level {
  ...
  protected void decorateBlock(int x, int y, Block block, int col) {
    ...
    if (col == 0xff0000) addEntity(new ZombieGuardEntity(x, y));
    if (col == 0xff0001) addEntity(new GuardEntity(x, y));
    if (col == 0xff0003) addEntity(new GuardBossEntity(x, y));
    if (col == 0xff0004) addEntity(new MiniBossGuardEntity(x, y));
    if (col == 0xff0005) addEntity(new BatEntity(x, y));
    if (col == 0xff0006) addEntity(new GuardWithPistolEntity(x, y));
  ...
  protected Block getBlock(int x, int y, int col) {
    ...
    if (col == 0xa3a345) return new AmmoBlock();
    if (col == 0xcd9e9e) return new HealthBlock();
  ...
  private static final Map<String, Level> loaded = new HashMap<>();
  ...
  private static Level byName(String name) {
    try {
      ...
      return (Level) Class.forName("com.example.escape.level." + name + "Level").newInstance();
    } catch (Exception e) {
      throw new RuntimeException(e);
    }
  }
