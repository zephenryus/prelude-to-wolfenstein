...
import com.example.escape.gui.PoofSprite;
...
public class EnemyEntity extends Entity {
  ...
  public boolean use(Entity source, Item item) {
    ...
    if (item != Item.knife) return false;

    System.out.println(item);

    hurt(Math.sin(source.rot), Math.cos(source.rot), 0);
  ...
  protected void collide(Entity entity) {
    if (entity instanceof Bullet) {
      ...
      entity.remove();
      hurt(entity.xa, entity.za, ((Bullet) entity).dmg);
    }
    if (entity instanceof Knife) {
      Knife knife = (Knife) entity;
      if (knife.owner.getClass() == this.getClass()) {
        return;
      }
      if (hurtTime > 0) return;
      entity.remove();
      hurt(entity.xa, entity.za, ((Knife) entity).dmg);
    }
