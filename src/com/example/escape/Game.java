...
import com.example.escape.level.block.LadderBlock;
import com.example.escape.menu.*;
...
public class Game {
  ...
  public void newGame() {
    Level.clear();
    level = Level.loadLevel(this, "floor1");

    player = new Player();
    player.addLoot(Item.knife);
    player.level = level;
    level.player = player;
    player.x = level.xSpawn;
    player.z = level.ySpawn;
    level.addEntity(player);
    player.rot = Math.PI - 1.3;
  }
  ...
  public void tick(boolean[] keys) {
    ...
    boolean turnLeft = (!strafe && lk);
    boolean turnRight = (!strafe && rk);
    ...
    boolean action = keys[KeyEvent.VK_E];
